...
// async function runSuiToEvmSwap() {
//     log.title('🔄 Cross-Chain Swap Demo: Sui → Arbitrum');
...
//     log.info('Sui User Address', suiUserAddress);
//     log.info('EVM Resolver Address', evmResolverWallet.address);
...
//         suiUserKeypair,
//         suiResolverAddress,
//         evmResolverWallet.address
//     );

//     const evmResolver = new Wallet(evmResolverWallet, evmProvider);
//     const evmResolverContract = new Resolver(config.evm.escrowFactory, config.evm.resolverContract);

//     // === Step 2: Create Sui wallet ===
//     log.step(2, 'Creating Sui wallet and source escrow');

//     const secret = uint8ArrayToHex(randomBytes(32));
//     const hashlock = keccak256(secret);
//     const salt = Sdk.randBigInt(1000n).toString();

//     const orderHash = keccak256(Buffer.from(secret)); // could be more structured
//     const makingAmount = parseUnits('1', 6); // 1 USDC
//     const takingAmount = parseUnits('0.99', 6); // min 0.99 USDC
//     const srcSafetyDeposit = parseUnits('0.01', 9);
//     const dstSafetyDeposit = parseUnits('0.01', 18);
//     const tokenType = config.sui.tokens.USDC;
//     const timelocks = {
//         srcWithdrawal: 300_000n,
//         srcPublicWithdrawal: 600_000n,
//         srcCancellation: 900_000n,
//         srcPublicCancellation: 1_200_000n,
//         dstWithdrawal: 250_000n,
//         dstPublicWithdrawal: 550_000n,
//         dstCancellation: 850_000n
//     };

//     const walletId = await suiResolver.createWallet(
//         orderHash,
//         salt,
//         'USDC',
//         'USDC',
...
//         false,
//         1,
//         makingAmount + srcSafetyDeposit,
//         timelocks
//     );

//     const walletInfo = await suiResolver.getWalletInfo(walletId);
//     if (!walletInfo) throw new Error('Failed to fetch wallet info');

//     const secretIndex = suiResolver.calculateSecretIndex(walletInfo, makingAmount);
//     const merkle = SuiResolver.buildMerkleTree([secret]);
//     const proof = merkle.proofs.get(secretIndex) ?? [];

//     const srcEscrowId = await suiResolver.deploySrc(
//         walletId,
//         hashlock,
//         secretIndex,
//         proof,
...
//     log.success(`Sui → Source escrow created: ${srcEscrowId}`);

//     // === Step 3: Deploy destination escrow on Arbitrum ===
//     log.step(3, 'Creating destination escrow on Arbitrum');

//     const immutables = Sdk.Immutables.new({
//         orderHash,
//         hashLock: Sdk.HashLock.fromString(hashlock),
//         maker: new Address(suiUserAddress),
//         taker: new Address(evmResolverWallet.address),
//         token: new Address(config.evm.tokens.USDC.address),
//         amount: takingAmount,
//         safetyDeposit: dstSafetyDeposit,
//         timeLocks: Sdk.TimeLocks.fromBigInt(timelocks)
//     });

//     const dstTx = await evmResolver.send(
//         evmResolverContract.deployDst(immutables)
//     );

//     const receipt = await evmProvider.getTransactionReceipt(dstTx.txHash);
//     if (!receipt || receipt.status !== 1) throw new Error('Destination escrow failed to deploy');

//     log.tx('Dst Escrow Tx', dstTx.txHash);
//     log.success('Destination escrow deployed on Arbitrum');

//     // === Step 4: Share secret and withdraw ===
//     log.step(4, 'Sharing secret and withdrawing funds');

//     // Withdraw on EVM (resolver)
//     const escrowFactory = new EscrowFactory(evmProvider, config.evm.escrowFactory);
//     const dstImpl = await escrowFactory.getDestinationImpl();
//     const dstEscrowAddress = new Sdk.EscrowFactory(new Address(config.evm.escrowFactory)).getDstEscrowAddress(
//         immutables,
//         dstImpl
//     );

//     const evmWithdrawTx = await evmResolver.send(
//         evmResolverContract.withdraw('dst', dstEscrowAddress, secret, immutables)
//     );

//     log.tx('Withdraw Tx', evmWithdrawTx.txHash);
//     log.success('Resolver withdrew funds on Arbitrum');

//     // Withdraw on Sui (user)
//     await suiResolver.withdraw(srcEscrowId, 'src', secret);
//     log.success('User withdrew funds on Sui');

//     // === Step 5: Done ===
//     log.title('✅ Swap Completed Successfully');
//     console.log(chalk.white('\n📊 Summary:'));
//     console.log(chalk.gray('  • ') + chalk.white('From:'), chalk.cyan('1 USDC on Sui'));
//     console.log(chalk.gray('  • ') + chalk.white('To:'), chalk.cyan('0.99 USDC on Arbitrum'));
//     console.log(chalk.gray('  • ') + chalk.white('Sui Wallet:'), chalk.gray(walletId));
//     console.log(chalk.gray('  • ') + chalk.white('Sui Escrow:'), chalk.gray(srcEscrowId));
//     console.log(chalk.gray('  • ') + chalk.white('Arbitrum Escrow:'), chalk.gray(dstEscrowAddress.toString()));
// }
